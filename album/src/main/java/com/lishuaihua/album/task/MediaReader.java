...
public class MediaReader {
     ...
     */
    @WorkerThread
    private void scanImageFileOne(Map<String, AlbumFolder> albumFolderMap) {
        ContentResolver contentResolver = mContext.getContentResolver();
        Cursor cursor = contentResolver.query(MediaStore.Images.Media.EXTERNAL_CONTENT_URI,
                IMAGES,
                null,
...
//        allFileFolder.setName(mContext.getString(R.string.all_images));
//
//        scanImageFile(albumFolderMap, allFileFolder);
//
        scanImageFileOne(albumFolderMap);
...
//        albumFolders.add(allFileFolder);
       ...
       String camera =  Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DCIM).getAbsolutePath();
        String myCamera = "";
        if(camera!=null){
            File file =  new File(camera);
            if(file!=null) {
                for(String string:file.list()){
                    if(string.equals("Camera") || string.equals("CAMERA") ||string.equals("camera")){
                        myCamera = string;
                    }
                }
            }
        ...
        if(!myCamera.equals("")){
            for (Map.Entry<String, AlbumFolder> folderEntry : albumFolderMap.entrySet()) {
                if(myCamera.equals(folderEntry.getKey())){
    ...
    public ArrayList<AlbumFolder> getAllVideo() {
        ...
        AlbumFolder allFileFolder = new AlbumFolder();
        allFileFolder.setChecked(true);
        allFileFolder.setName(mContext.getString(R.string.all_videos));
