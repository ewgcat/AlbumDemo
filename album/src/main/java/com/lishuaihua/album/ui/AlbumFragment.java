...
import com.lishuaihua.album.ui.adapter.AlbumFileAdapter;
...
import com.lishuaihua.album.task.MediaReadTask;
import com.lishuaihua.album.task.PathConvertTask;
import com.lishuaihua.album.util.AlbumUtils;
import com.lishuaihua.album.util.DisplayUtils;
import com.lishuaihua.album.widget.divider.Divider;
import com.lishuaihua.fragment.NoFragment;
import com.lishuaihua.mediascanner.MediaScanner;
...
public class AlbumFragment extends NoFragment {
    ...
    private void initializeWidget() {
        ...
        if (mWidget.getStyle() == Widget.STYLE_LIGHT) {
            ...
            AlbumUtils.setDrawableTint(mFinishMenuItem.getIcon(), ContextCompat.getColor(getContext(), R.color.IconDark));
            CharSequence title = mFinishMenuItem.getTitle();
            title = AlbumUtils.getColorText(title,
                    0,
                    title.length(),
                    ContextCompat.getColor(getContext(), R.color.FontDark));
            ...
            if (hasCheckSize >= mLimitCount) {
                ...
                switch (mFunction) {
                    case Album.FUNCTION_CHOICE_IMAGE: {
                        messageRes = R.plurals.check_image_limit_camera;
                        break;
                    }
                    case Album.FUNCTION_CHOICE_VIDEO: {
                        messageRes = R.plurals.check_video_limit_camera;
                        break;
                    }
                    case Album.FUNCTION_CHOICE_ALBUM:
                    default: {
                        messageRes = R.plurals.check_limit_camera;
                        break;
                    }
                }
                Toast.makeText(
                ...
                switch (mFunction) {
                    ...
                    default: {
                        if (mCameraPopupMenu == null) {
                            ...
                            mCameraPopupMenu.getMenuInflater().inflate(R.menu.menu_item_camera, mCameraPopupMenu.getMenu());
    ...
    private PopupMenu.OnMenuItemClickListener mCameraItemMenuClickListener = new PopupMenu.OnMenuItemClickListener() {
        ...
        public boolean onMenuItemClick(MenuItem item) {
            ...
            if (id == R.id.menu_camera_image) {
                Album.camera(getContext())
                        .image()
                        .requestCode(REQUEST_CODE_CAMERA_IMAGE)
                        .onResult(mCameraAction)
                        .start();
            } else if (id == R.id.menu_camera_video) {
                Album.camera(getContext())
                        .video()
                        .requestCode(REQUEST_CODE_CAMERA_VIDEO)
                        .onResult(mCameraAction)
            ...
            if (albumFile.isEnable()) {
                ...
                if (mFilterVisibility)
                    ...
                    Toast.makeText(getContext(),
                            getString(R.string.take_file_unavailable),
                            Toast.LENGTH_LONG).show();
            }
        }
    };
    ...
    private void onAlbumResult() {
        ...
        if (allSize > 0 && checkSize == 0) {
            ...
            switch (mFunction) {
                case Album.FUNCTION_CHOICE_IMAGE: {
                    messageRes = R.string.check_image_little;
                    break;
                }
                case Album.FUNCTION_CHOICE_VIDEO: {
                    messageRes = R.string.check_video_little;
                    break;
                }
                case Album.FUNCTION_CHOICE_ALBUM:
                default: {
                    messageRes = R.string.check_little;
                    break;
                }
            }
